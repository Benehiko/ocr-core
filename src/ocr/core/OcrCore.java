...
public class OcrCore {
    ...
    public String[] process_image_array(BufferedImage bi) throws CvHandler, IOException, TesseractException{
        ...
        IplImage img = OcrConvert.convertBufferedToIpl(bi);

        //Resize image to fit standards
        img = ImageProcessing.resize(img);
        
        //Get pre-processing shape
        IplImage shape_image = OcrPreProcessing.refine_shape_finder(img);
        ...
        BufferedImage refined = OcrPreProcessing.refine_image(OcrConvert.convertIplToBuffered(img));
