 ...
 * 
 * This is version 2 of OcrShapes and is currently working
...
public class OcrShapes {
    ...
    /**
     * Recognise shapes in an image and return the drawn image
     * @param img_src IplImage
     * @return IplImage
     */
    public IplImage recognise_shapes(IplImage img_src){
        IplImage img_return = null;
        //get size of image
        opencv_core.CvSize sz = cvSize(img_src.width() & -2, img_src.height() & -2);
        IplImage img = cvCloneImage(img_src); // make a copy of input image
               
        //Get image with shapes recognised on it 
        img_return = drawSquares(img, find_contours(img));
    ...
    /**
     * Method used to get "mini" images from iplimage for text extraction 
     * @param img IplImage
     * @param img_original
     * @return ArayList of BufferedImages
     */
    public ArrayList<BufferedImage> get_images(IplImage img, IplImage img_original){
        CvSeq squares = find_contours(img);
        ArrayList<IplImage> arrImg = extract_images(img_original, squares);
        ArrayList<BufferedImage> arrBuffered = new ArrayList<>();
        
        //functional operator for each arrImage to ipl
        arrImg.forEach((ipl) -> {
            
            arrBuffered.add(OcrConvert.convertIplToBuffered(ipl));
        });
        return arrBuffered;
    }
    
    /**
     * Helper method for find_contours
     * @param pt1
     * @param pt2
     * @param pt0
     * @return 
    ...
    //Find contours 
    public CvSeq find_contours(IplImage img_src){
        ...
        //create the squares vertices
        CvSeq squares = cvCreateSeq(0, Loader.sizeof(CvSeq.class), Loader.sizeof(opencv_core.CvPoint.class), strg);
        cvFindContours(img, strg, contours, Loader.sizeof(opencv_core.CvContour.class), CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE, cvPoint(0,0));
             
         while (contours != null && !contours.isNull()) {
                    // approximate contour with accuracy proportional
                    // to the contour perimeter
                    // Java translation: moved into the loop
                    CvSeq result = cvApproxPoly(contours, Loader.sizeof(opencv_core.CvContour.class), strg, CV_POLY_APPROX_DP, cvContourPerimeter(contours)*0.02, 0);
                    // square contours should have 4 vertices after approximation
                    // relatively large area (to filter out noisy contours)
                    // and be convex.
                    // Note: absolute value of an area is used because
    ...
    /**
     * Extract images from the recognised shapes.
     * Still have issues with this method.
     * @param img
     * @param squares
     * @return 
     */
    private ArrayList<IplImage> extract_images(IplImage img, CvSeq squares){
        ...
        //source: https://stackoverflow.com/questions/12586180/cropping-iplimage-most-effectively
        ...
        for(int i=0; i <= squares.total(); i +=4){
            ...
            IplImage cropped = new IplImage(temp_img);
            cvCopy(temp_img,cropped, null);
